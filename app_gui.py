...
class Config:
    ...
    XML_DIR_DEFAULT = "./renamed_highlighted_cwrgm_xml/"
    PAGE_TITLE = "📜 Historical Document Analyzer"
...
def get_available_search_types() -> List[str]:
    ...
    # Conditionally add topic modeling searches if available
    if st.session_state.spacy_nlp_instance:
        search_types.append("Smart Search (spaCy)")
    
    if st.session_state.sentence_model_instance:
        search_types.append("Semantic Search")
    
    if st.session_state.qa_pipeline_instance and st.session_state.sentence_model_instance:
        search_types.append("Question Answering")
    
    if st.session_state.sentence_model_instance:
        search_types.append("Hybrid Search")
    
    # Topic modeling
    try:
        import gensim
        if chat.global_lda_model:
            search_types.extend(["Topic ID", "Topic Terms"])
    except ImportError:
        pass
...
def get_search_placeholder(search_type: str) -> str:
    ...
    placeholders = {
        ...
        "Subject": "Enter a subject tag from your Excel file"
    }
    return placeholders.get(search_type, "Enter your query...")
...
def get_search_help(search_type: str) -> str:
    ...
    help_texts = {
        ...
        "Sender": "Search for letters from a specific person.",
        "Recipient": "Search for letters to a specific person.",
        "Year": "Find all letters from a specific year (e.g., 1862).",
        "Place": "Find letters mentioning a specific location.",
        "Topic": "Explore documents related to a specific topic.",
        "Subject": "Find documents with a specific subject tag."
    }
    return help_texts.get(search_type, "Enter your query and hit search.")
